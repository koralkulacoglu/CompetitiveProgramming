...
void solve() {
	int n, d; cin >> n >> d;
	int a, b; cin >> a >> b;
	vll m(n); FOR (i, 0, n) cin >> m[i];

	sort(m.begin(), m.end());

	// only 'a' bottles will ever be modified optimally
	int untouched = n - a;
	modnum<MOD> moonies = 0;
	FOR (i, 0, untouched) moonies += m[i] * m[i];

	vll contested(m.begin() + untouched, m.end());
	int c = sz(contested);

	// now we want subtract 'bd' milk s.t. contested values are as close as possible
	// we can binary search to find this "closest" value
	
	ll target = ll(b) * d;

	auto getRemoved = [&](ll treshold) -> ll {
		ll total = 0;
		FOR (i, 0, c) {
			ll prev = contested[i];
			ll cur = contested[i] + d;
			ll next = max(prev, min(cur, treshold));
			total += cur - next;
		}
		return total;
	};
	...
	while (left <= right) {
		...
		if (getRemoved(mid) >= target) left = mid + 1;
		else right = mid - 1;
	}
    ...
    ll curRemoved = getRemoved(best);
    ll extraToRemove = target - curRemoved;
    FOR (i, 0, c) {
        ll prev = contested[i];
        ll cur = contested[i] + d;
        ll next = max(prev, min(cur, best));

        if (extraToRemove > 0 && next == best && next > prev) {
            next--;
            extraToRemove--;
        }
        moonies += next * next;
