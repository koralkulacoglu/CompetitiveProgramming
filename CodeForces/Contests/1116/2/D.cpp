...
if its a palindrome flipping it does nothing

if we flip something with a palindrome in it, the palindrome still cannot be
flipped

answer = # non-palindrome flips + 1???? no

some sort of dp??

*/

modnum<MOD> fact[MX], ifact[MX];

void initFact() {
  fact[0] = 1;
  FOR(i, 1, MX) fact[i] = fact[i - 1] * i;
  ifact[MX - 1] = fact[MX - 1].inv();
  F0Rd(i, MX - 1) ifact[i] = ifact[i + 1] * (i + 1);
}

modnum<MOD> choose(int a, int b) {
  if (b < 0 || b > a)
    return 0;
...
void solve() {
  ...
  int adj0 = 0, adj1 = 0, groups0 = s[0] == '0', groups1 = s[0] == '1';
  FOR(i, 1, n) {
    if (s[i] == s[i - 1]) {
      adj0 += s[i] == '0';
      adj1 += s[i] == '1';
  ...
  modnum<MOD> ans = 1;
  if (groups0)
    ans *= choose(adj0 + groups0 - 1, groups0 - 1);
  if (groups1)
    ans *= choose(adj1 + groups1 - 1, groups1 - 1);
