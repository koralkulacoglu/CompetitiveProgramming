...
public:
    int countTrapezoids(vector<vector<int>>& points) {
        if (fact.empty()) {
            fact.push_back(1);
            fact.push_back(1);
            for (int i=2; i<=1e5; i++) fact.push_back(fact[fact.size()-1]*i);
        }

        unordered_map<int, int> line;
        for (vector<int> point : points) line[point[1]]++;

        modnum<MOD> total = 0;
        for (auto i : line) total = total + nCr(i.second, 2);

        modnum<MOD> ans = 0;
        for (auto i : line) {
            modnum<MOD> choices = nCr(i.second, 2);
            total -= choices;
            ans += choices * total;
        }
